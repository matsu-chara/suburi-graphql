...
func (s shortMapper) Mutation_postMessage(ctx context.Context, user string, text string) (*Message, error) {
	return s.r.Mutation().PostMessage(ctx, user, text)
}

func (s shortMapper) Query_messages(ctx context.Context) ([]Message, error) {
	return s.r.Query().Messages(ctx)
}

func (s shortMapper) Query_users(ctx context.Context) ([]string, error) {
	return s.r.Query().Users(ctx)
}

func (s shortMapper) Subscription_messagePosted(ctx context.Context, user string) (<-chan Message, error) {
	return s.r.Subscription().MessagePosted(ctx, user)
}

func (s shortMapper) Subscription_userJoined(ctx context.Context, user string) (<-chan string, error) {
	return s.r.Subscription().UserJoined(ctx, user)
}
